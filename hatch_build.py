"""Hatch build hook that bundles the webui (Vite) into teai_builder/web/dist.
...
- No-op when `webui/package.json` is absent (e.g. installing from an sdist that
  already contains a prebuilt `teai_builder/web/dist/`).
- Skips when `TEAI_BUILDER_SKIP_WEBUI_BUILD=1` is set.
- Skips when `teai_builder/web/dist/index.html` already exists, unless
  `TEAI_BUILDER_FORCE_WEBUI_BUILD=1` is set.
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        dist_dir = root / "teai_builder" / "web" / "dist"
        ...
        if os.environ.get("TEAI_BUILDER_SKIP_WEBUI_BUILD") == "1":
            self.app.display_info("[webui-build] skipped via TEAI_BUILDER_SKIP_WEBUI_BUILD=1")
            return

        if not package_json.is_file():
            self.app.display_info(
                "[webui-build] no webui/ source tree, assuming prebuilt teai_builder/web/dist/"
            )
            return

        force = os.environ.get("TEAI_BUILDER_FORCE_WEBUI_BUILD") == "1"
        if index_html.is_file() and not force:
            self.app.display_info(
                f"[webui-build] reusing existing build at {dist_dir} "
                "(set TEAI_BUILDER_FORCE_WEBUI_BUILD=1 to rebuild)"
            )
            return
        ...
        if runner is None:
            raise RuntimeError(
                ...
                "install one or set TEAI_BUILDER_SKIP_WEBUI_BUILD=1 to bypass."
            )
