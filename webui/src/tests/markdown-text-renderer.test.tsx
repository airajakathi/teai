...
describe("MarkdownTextRenderer", () => {
  ...
  it("renders local file links as previewable file references", () => {
    ...
    render(
      <MarkdownTextRenderer onOpenFilePreview={onOpenFilePreview}>
        {"Edited [hook.py](/Users/test/project/teai_builder/agent/hook.py:12)"}
    ...
    expect(reference).toHaveAttribute(
      "aria-label",
      "/Users/test/project/teai_builder/agent/hook.py",
    );
  ...
  it("does not wrap complete fenced code blocks in an extra pre", () => {
    const { container } = render(
      <MarkdownTextRenderer highlightCode={false}>
        {"当前目录:\n\n```text\n/Users/renxubin/.teai_builder/workspace\n```"}
      </MarkdownTextRenderer>,
    );

    expect(screen.getByText("/Users/renxubin/.teai_builder/workspace")).toBeInTheDocument();
  ...
  it("renders markdown videos as inline players", () => {
    render(<MarkdownTextRenderer>![teai_builder-intro.mp4](/api/media/sig/video)</MarkdownTextRenderer>);

    const video = screen.getByLabelText("Video attachment: teai_builder-intro.mp4");
    expect(video.tagName).toBe("VIDEO");
    expect(video).toHaveAttribute("src", "/api/media/sig/video");
    expect(video).toHaveAttribute("controls");
    expect(screen.queryByRole("img", { name: "teai_builder-intro.mp4" })).not.toBeInTheDocument();
  });
