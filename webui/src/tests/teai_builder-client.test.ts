...
/**
 * Minimal fake WebSocket implementing the subset TeaiBuilderClient touches.
...
afterEach(() => {
  Reflect.deleteProperty(window, "teai_builderHost");
  vi.useRealTimers();
});

describe("TeaiBuilderClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("teai_builder-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("teai_builder-host://engine/");
    expect(lastSocket().url).toBe("host:teai_builder-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    Object.defineProperty(window, "teai_builderHost", {
      configurable: true,
      value: {
        openSocket,
    ...
    expect(openSocket).toHaveBeenCalledWith("teai_builder-host://engine/");
