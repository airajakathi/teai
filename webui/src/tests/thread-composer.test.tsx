...
describe("ThreadComposer", () => {
  ...
  it("prioritizes stop as an immediate slash action while streaming", () => {
    ...
    expect(window.localStorage.getItem("teai_builder.webui.slashCommandRecents")).toBeNull();
  });

  it("orders recent slash commands first for the blank slash menu", () => {
    window.localStorage.setItem("teai_builder.webui.slashCommandRecents", JSON.stringify(["/history"]));
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
