...
import type { StreamError } from "@/lib/teai_builder-client";
import type {
  InboundEvent,
...
export function useTeaiBuilderStream(
  chatId: string | null,
    ...
    const handle = (ev: InboundEvent) => {
      ...
      if (ev.event === "message") {
        ...
        if (ev.kind === "tool_hint" || ev.kind === "progress") {
          // Canvas workspace events – dispatch and skip trace rendering if no other content.
          if (ev.agent_ui?.kind === "canvas") {
            window.dispatchEvent(
              new CustomEvent("teai_builder:canvas", {
                detail: { chatId: ev.chat_id, data: ev.agent_ui.data },
              }),
            );
            if (!ev.text && !ev.tool_events?.length) return;
          }
