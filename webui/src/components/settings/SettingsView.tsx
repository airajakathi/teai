...
const LanguageSwitcher = lazy(async () => {
  ...
  return { default: mod.LanguageSwitcher };
});
const preloadLanguageSwitcher = () => import("@/components/LanguageSwitcher");

const SkillsCatalogSettings = lazy(async () => {
  const mod = await import("@/components/settings/SkillsCatalogSettings");
  return { default: mod.SkillsCatalogSettings };
});
const preloadSkillsCatalogSettings = () => import("@/components/settings/SkillsCatalogSettings");

const LazyAppsCatalogSettings = lazy(async () => {
  const mod = await import("@/components/settings/AppsCatalogSettings");
  return { default: mod.AppsCatalogSettings };
});
const preloadAppsCatalogSettings = () => import("@/components/settings/AppsCatalogSettings");

const LazyAdvancedSettingsSection = lazy(async () => {
  const mod = await import("@/components/settings/AdvancedSettingsSection");
  return { default: mod.AdvancedSettingsSection };
});
const preloadAdvancedSettingsSection = () =>
  import("@/components/settings/AdvancedSettingsSection");

const LazyRuntimeSettingsSection = lazy(async () => {
  const mod = await import("@/components/settings/RuntimeSettingsSection");
  return { default: mod.RuntimeSettingsSection };
});
const preloadRuntimeSettingsSection = () =>
  import("@/components/settings/RuntimeSettingsSection");

const TokenUsageHeatmap = lazy(async () => {
  const mod = await import("@/components/settings/TokenUsageHeatmap");
  return { default: mod.TokenUsageHeatmap };
});
const preloadTokenUsageHeatmap = () => import("@/components/settings/TokenUsageHeatmap");
...
const LOCAL_PREFS_STORAGE_KEY = "teai_builder-webui.settings-preferences";
...
const DEFAULT_CHANNEL_SETTINGS_FORM: ChannelSettingsDraft = {
  sendProgress: true,
  sendToolHints: false,
  showReasoning: true,
...
function SettingsInlineFallback({
  label,
  className,
}: {
  label: string;
  ...
  return (
    <div
      aria-label={label}
      className={cn(
        "rounded-[16px] border border-border/45 bg-card/70 p-4 shadow-sm",
        className,
      )}
    >
...
function channelSettingsFormFromPayload(payload: SettingsPayload): ChannelSettingsDraft {
  return {
    sendProgress: payload.channels?.send_progress ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendProgress,
    sendToolHints: payload.channels?.send_tool_hints ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendToolHints,
    showReasoning: payload.channels?.show_reasoning ?? DEFAULT_CHANNEL_SETTINGS_FORM.showReasoning,
    extractDocumentText:
      payload.channels?.extract_document_text ?? DEFAULT_CHANNEL_SETTINGS_FORM.extractDocumentText,
    sendMaxRetries: payload.channels?.send_max_retries ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendMaxRetries,
  };
}
...
export function SettingsView({
  ...
  useEffect(() => {
    if (activeSection === "overview") {
      void preloadTokenUsageHeatmap();
      return;
    }
    if (activeSection === "appearance") {
      void preloadLanguageSwitcher();
      return;
    }
    if (activeSection === "skills") {
      void preloadSkillsCatalogSettings();
      return;
    }
    if (activeSection === "apps") {
      void preloadAppsCatalogSettings();
      return;
    }
    if (activeSection === "runtime") {
      void preloadRuntimeSettingsSection();
      return;
    }
    if (activeSection === "advanced") {
      void preloadAdvancedSettingsSection();
    }
  }, [activeSection]);
  ...
  const [channelSettingsForm, setChannelSettingsForm] = useState<ChannelSettingsDraft>(() =>
    initialSettings ? channelSettingsFormFromPayload(initialSettings) : DEFAULT_CHANNEL_SETTINGS_FORM,
  );
  ...
  const applyPayload = useCallback((payload: SettingsPayload) => {
    ...
    setChannelSettingsForm(channelSettingsFormFromPayload(payload));
  ...
  const runtimeDirty = useMemo(() => {
    ...
    return (
      ...
      channelSettingsForm.sendProgress !== (settings.channels?.send_progress ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendProgress) ||
      channelSettingsForm.sendToolHints !== (settings.channels?.send_tool_hints ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendToolHints) ||
      channelSettingsForm.showReasoning !== (settings.channels?.show_reasoning ?? DEFAULT_CHANNEL_SETTINGS_FORM.showReasoning) ||
      channelSettingsForm.extractDocumentText !== (
        settings.channels?.extract_document_text ?? DEFAULT_CHANNEL_SETTINGS_FORM.extractDocumentText
      ) ||
      channelSettingsForm.sendMaxRetries !== (
        settings.channels?.send_max_retries ?? DEFAULT_CHANNEL_SETTINGS_FORM.sendMaxRetries
      )
    );
  }, [channelSettingsForm, form, settings]);
  ...
  const saveRuntimeSettings = async () => {
    ...
    try {
      const payload = await updateSettings(token, {
        ...
        channelSendProgress: channelSettingsForm.sendProgress,
        channelSendToolHints: channelSettingsForm.sendToolHints,
        channelShowReasoning: channelSettingsForm.showReasoning,
        channelExtractDocumentText: channelSettingsForm.extractDocumentText,
        channelSendMaxRetries: channelSettingsForm.sendMaxRetries,
      });
      applyPayload(payload);
    ...
    switch (activeSection) {
      ...
      case "apps":
        return (
          <Suspense
            fallback={
              <SettingsInlineFallback
                label={text("settings.apps.loadingCatalog", "Loading apps catalog")}
                className="min-h-[320px]"
              />
            }
          >
            <LazyAppsCatalogSettings
              cliApps={cliApps}
      ...
      case "skills":
        return (
          <Suspense
            fallback={
              <SettingsInlineFallback
                label={text("settings.skills.loading", "Loading skills catalog")}
      ...
      case "runtime":
        return (
          <Suspense
            fallback={
              <SettingsInlineFallback
                label={text("settings.runtime.loading", "Loading runtime settings")}
                className="min-h-[360px]"
              />
            }
          >
            <LazyRuntimeSettingsSection
              form={form}
              channelForm={channelSettingsForm}
              settings={settings}
              dirty={runtimeDirty}
              saving={saving}
              ...
              isRestarting={isRestarting || hostEngineApplying}
              requiresRestartPending={pendingRestartSections.runtime}
              onBotNameChange={(value) => setForm((prev) => ({ ...prev, botName: value }))}
              onBotIconChange={(value) => setForm((prev) => ({ ...prev, botIcon: value }))}
              onTimezoneChange={(value) => setForm((prev) => ({ ...prev, timezone: value }))}
              onChannelSendProgressChange={(value) =>
                setChannelSettingsForm((prev) => ({ ...prev, sendProgress: value }))
              }
              onChannelSendToolHintsChange={(value) =>
                setChannelSettingsForm((prev) => ({ ...prev, sendToolHints: value }))
              }
              onChannelShowReasoningChange={(value) =>
                setChannelSettingsForm((prev) => ({ ...prev, showReasoning: value }))
              }
              onChannelExtractDocumentTextChange={(value) =>
                setChannelSettingsForm((prev) => ({ ...prev, extractDocumentText: value }))
              }
              onChannelRetriesChange={(value) =>
                setChannelSettingsForm((prev) => ({ ...prev, sendMaxRetries: value }))
              }
            />
          </Suspense>
        );
      case "advanced":
        return (
          <Suspense
            fallback={
              <SettingsInlineFallback
                label={text("settings.advanced.loading", "Loading advanced settings")}
                className="min-h-[320px]"
              />
            }
          >
            <LazyAdvancedSettingsSection
    ...
    <div className="space-y-7">
      <section>
        <Suspense
          fallback={
            <SettingsInlineFallback
              label={tx("settings.usage.loading", "Loading token usage")}
      ...
      <section>
        ...
        <SettingsGroup>
          <ReadOnlyRow
            title={tx("settings.about.version", "Version")}
            value={settings.version?.current ? `v${settings.version.current}` : "teai_builder"}
          />
        </SettingsGroup>
      </section>
    </div>
        ...
        <SettingsGroup>
          ...
          >
            <Suspense
              fallback={
                <SettingsInlineFallback
                  label={tx("settings.language.loading", "Loading language switcher")}
