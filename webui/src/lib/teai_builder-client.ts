...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "teai_builder-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('teai_builder_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('teai_builder_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("teai_builder_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface TeaiBuilderClientOptions {
  url: string;
  reconnect?: boolean;
 ...
 */
export class TeaiBuilderClient {
  ...
  constructor(private options: TeaiBuilderClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[teai_builder ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[teai_builder ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - TeaiBuilderClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
