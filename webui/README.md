...
The WebUI is the browser workbench served by `teai_builder gateway`. If you installed `teai_builder-ai` from PyPI, the WebUI bundle is already included; this `webui/` source tree is only needed when you are changing the frontend.
...
If you installed teai_builder via `python -m pip install teai_builder-ai`, the WebUI is **already bundled** in the wheel. You do **not** need Node.js, Bun, Vite, or anything in this directory unless you are changing the WebUI source code.
...
If the shell cannot find `teai_builder`, use the module form from the same Python environment:

```bash
python -m teai_builder agent -m "Hello!"
```

Then merge this WebSocket snippet into your existing `~/.teai_builder/config.json` instead of replacing the whole file:
...
teai_builder/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install teai_builder from source
...
In `~/.teai_builder/config.json`, merge:
...
```bash
TEAI_BUILDER_API_URL=http://127.0.0.1:9000 bun run dev
```

### Access from another device (LAN)

To use the WebUI from another device on the same network, set `host` to `"0.0.0.0"` and configure a `token` or `tokenIssueSecret` in `~/.teai_builder/config.json`:
...
```bash
cd webui
bun run build          # writes to ../teai_builder/web/dist
```
