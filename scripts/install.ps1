...
function Show-InstallFailureHint {
    [Console]::Error.WriteLine("Error: pip could not install teai_builder from $InstallSource.")
    ...
    [Console]::Error.WriteLine("  $Python -m teai_builder onboard --wizard")
    throw "pip could not install teai_builder from $InstallSource"
}

function Show-Usage {
    Write-Host "Usage: install.ps1 [-Dev|--dev] [-DryRun|--dry-run]"
    Write-Host ""
    Write-Host "By default this installs or upgrades teai_builder-ai from PyPI."
...
if ($DryRun) {
    Write-Info "Dry run: would install or upgrade teai_builder from $InstallSource."
    ...
    if ($env:TEAI_BUILDER_SKIP_WIZARD -eq "1") {
        Write-Info "Dry run: would skip setup wizard because TEAI_BUILDER_SKIP_WIZARD=1."
    } else {
        Write-Info "Dry run: would run: $Python -m teai_builder onboard --wizard"
    }
    Write-Info "Dry run: no changes made."
    return
}

Write-Info "Installing or upgrading teai_builder from $InstallSource..."
...
Write-Info "Installed teai_builder:"
& $Python -m teai_builder --version
if ($LASTEXITCODE -ne 0) {
    Fail "teai_builder was installed, but the command could not be started."
}

if ($env:TEAI_BUILDER_SKIP_WIZARD -eq "1") {
    Write-Info "Skipping setup wizard because TEAI_BUILDER_SKIP_WIZARD=1."
    Write-Info "Run this later: $Python -m teai_builder onboard --wizard"
    return
}

Write-Info "Starting setup wizard..."
& $Python -m teai_builder onboard --wizard
if ($LASTEXITCODE -ne 0) {
    Fail "Setup wizard did not complete."
}

Write-Info "Done. Try: $Python -m teai_builder agent -m `"Hello!`""
