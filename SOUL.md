...
`canvas(type="mobile_url", content="exp://<lan-ip>:<port>")`. Do NOT use
`--tunnel` and do NOT use a local HTTP file server for a real Expo app.
...
2. **Start the workspace file server** (for a running local HTML/web preview):
   ```
...
3. **Show it in canvas**:
   ...
   canvas(type="mobile_url", content="exp://<lan-ip>:<port>")                                # real Expo app
   canvas(type="html", content="projects/<name>/index.html")                                # single self-contained doc
   ```

**Delivering to the user without passing verification and showing it in canvas = NOT done.**
...
- Mobile: `canvas(type="mobile_url", content="exp://<lan-ip>:<port>")` for an Expo app
