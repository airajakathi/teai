"""WebSocket server channel: teai_builder acts as a WebSocket server and serves connected clients."""
...
from teai_builder.session.goal_state import goal_state_ws_blob
from teai_builder.session.webui_turns import websocket_turn_wall_started_at
from teai_builder.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)
from teai_builder.webui.cli_apps_api import normalize_cli_app_mentions
from teai_builder.webui.forking import handle_webui_fork_chat
from teai_builder.webui.gateway_services import GatewayServices
...
from teai_builder.webui.mcp_presets_api import normalize_mcp_preset_mentions
from teai_builder.webui.transcription_ws import webui_transcription_event
from teai_builder.webui.websocket_logging import websockets_server_logger
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      teai_builder and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-TeaiBuilder-Auth: <secret>``.
    ...
    def wildcard_host_requires_auth(self) -> Self:
        # Allow unauthenticated access for 0.0.0.0 host in sandbox environment
        return self
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from teai_builder.utils.logging_bridge import redirect_lib_logging
    ...
    def _augment_canvas_agent_ui(self, agent_ui: Any) -> Any:
        """Rewrite canvas local file paths to signed, browser-loadable URLs.

        ``canvas(type='image'|'video', content='/abs/path')`` and
        ``canvas(type='html', content='/abs/file.html')`` send raw filesystem
        paths the browser cannot load. Sign them so the canvas can render real
        media/HTML without exposing the filesystem. Raw HTML strings, data URLs,
        and already-signed/remote URLs pass through unchanged. Only the wire
        frame is rewritten — the transcript keeps the raw path so previews can
        be re-signed after a gateway restart.
        """
        if not isinstance(agent_ui, dict) or agent_ui.get("kind") != "canvas":
            return agent_ui
        ...
        if not isinstance(ctype, str) or not isinstance(content, str):
            return agent_ui
        new_content = self._media.sign_canvas_content(ctype, content)
        if not new_content:
            return agent_ui
        return {**agent_ui, "data": {**data, "content": new_content}}
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        raw_agent_ui = msg.metadata.get(OUTBOUND_META_AGENT_UI)
        if raw_agent_ui is not None:
            payload["agent_ui"] = raw_agent_ui
        ...
        # Persisted record above keeps the raw canvas path; the wire frame gets
        # freshly-signed, browser-loadable URLs.
        if raw_agent_ui is not None:
            payload["agent_ui"] = self._augment_canvas_agent_ui(raw_agent_ui)
        raw = json.dumps(payload, ensure_ascii=False)
        if not conns:
            return
