...
from teai_builder.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.teai_builder/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'teai_builder channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"teai_builder-{uuid.uuid4().hex[:12]}"
