...
from teai_builder.cron.types import CronSchedule

if TYPE_CHECKING:
    from teai_builder.agent.tools.audio_generation import AudioGenerationToolConfig
    from teai_builder.agent.tools.cli_apps import CliAppsToolConfig
    from teai_builder.agent.tools.image_generation import ImageGenerationToolConfig
    from teai_builder.agent.tools.self import MyToolConfig
    from teai_builder.agent.tools.shell import ExecToolConfig
    from teai_builder.agent.tools.video_generation import VideoGenerationToolConfig
    from teai_builder.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by teai_builder.audio.transcription_registry.
    model: str | None = None
...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from teai_builder.providers.base import GenerationSettings
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.teai_builder/workspace"
    ...
    bot_name: str = "TeAi Builder"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("teai_builder.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("teai_builder.agent.tools.shell", "ExecToolConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("teai_builder.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("teai_builder.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("teai_builder.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
    video_generation: VideoGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("teai_builder.agent.tools.video_generation", "VideoGenerationToolConfig"),
    )
    audio_generation: AudioGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("teai_builder.agent.tools.audio_generation", "AudioGenerationToolConfig"),
    )
...
class Config(BaseSettings):
    ...
    def _match_provider(
        ...
        *,
        ...
        from teai_builder.providers.registry import (
            PROVIDERS,
            find_by_name,
        )
    ...
    model_config = ConfigDict(env_prefix="TEAI_BUILDER_", env_nested_delimiter="__")
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from teai_builder.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
    ...
    from teai_builder.agent.tools.shell import ExecToolConfig
    from teai_builder.agent.tools.video_generation import VideoGenerationToolConfig
    from teai_builder.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
