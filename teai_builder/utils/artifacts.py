...
from teai_builder.utils.helpers import detect_image_mime, ensure_dir
...
def store_generated_video_artifact(
    ...
    *,
    ...
    if not raw:
        raise ArtifactError("generated video payload was empty")
...
def store_generated_audio_artifact(
    ...
    *,
    ...
    """Persist generated speech/audio and sidecar metadata under the media root."""
    if not raw:
        raise ArtifactError("generated audio payload was empty")
    ext = _AUDIO_MIME_EXTENSIONS.get(mime)
    if ext is None:
        raise ArtifactError(f"unsupported audio MIME type: {mime}")
...
def generated_audio_tool_result(artifacts: list[dict[str, Any]]) -> str:
    """Return the compact structured result exposed to the LLM for audio."""
    return json.dumps(
        {
            "artifacts": artifacts,
            "next_step": (
                "Call the message tool with the artifact paths in the media parameter "
                "to deliver the audio to the user. Keep raw paths internal unless the "
...
def generated_video_tool_result(artifacts: list[dict[str, Any]]) -> str:
    """Return the compact structured result exposed to the LLM for videos."""
    return json.dumps(
        {
            "artifacts": artifacts,
            "next_step": (
                "Call the message tool with the artifact paths in the media parameter "
                "to deliver the videos to the user, or show one in the canvas with "
                "canvas(type=\"video\", content=\"<path>\"). Keep raw paths internal "
