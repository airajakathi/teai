"""Utility functions for teai_builder."""
...
_TOOL_RESULTS_DIR = ".teai_builder/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 teai_builder v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("teai_builder") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
    ...
    try:
        from teai_builder.utils.gitstore import GitStore
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the teai_builder package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("teai_builder") / "templates" / template_name
        if tpl.is_file():
