---
name: update-setup
description: One-time setup wizard for the teai_builder upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `teai_builder --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v teai_builder || true
python -m pip show teai_builder-ai || true
pipx list | sed -n '/teai_builder-ai/,+3p' || true
uv tool list | sed -n '/teai_builder-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how teai_builder was
installed.
...
`question: "Where is your teai_builder source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "teai_builder-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "teai_builder-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "teai_builder-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade teai_builder to the latest version. Triggers: upgrade teai_builder, update teai_builder, 升级teai_builder, 更新teai_builder."
---
...
4. Use `exec` to verify: `teai_builder --version`
5. Tell the user the new version. Say: "Run `/restart` to restart teai_builder and apply the update. If `/restart` is unavailable in this channel, restart the teai_builder process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade teai_builder' when you want to update."
