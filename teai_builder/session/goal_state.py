...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~teai_builder.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when this is a
    sustained-goal turn; ``None`` means use ``TEAI_BUILDER_LLM_TIMEOUT_S``. Pass in-memory
    ``metadata`` when the caller already holds :attr:`~teai_builder.session.manager.Session.metadata`
    for this turn.
    """
