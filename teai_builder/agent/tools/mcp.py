"""MCP client: connects to MCP servers and wraps their tools as native teai_builder tools."""
...
from teai_builder.bus.events import (
    INBOUND_META_RUNTIME_CONTROL,
    RUNTIME_CONTROL_ACK,
    RUNTIME_CONTROL_MCP_RELOAD,
    InboundMessage,
)
from teai_builder.security.network import validate_url_target
...
class MCPToolWrapper(_MCPWrapperBase):
    """Wraps a single MCP server tool as a teai_builder Tool."""
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only teai_builder Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only teai_builder Tool."""
    ...
    for item in structured[:8]:
        ...
        if configured_server_names is not None and raw_name not in configured_server_names:
            lines.append(
                ...
                "tell the user to restart teai_builder."
            )
            continue
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart teai_builder only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart teai_builder to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting teai_builder."
        else:
            message = "MCP config reloaded without restarting teai_builder."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart teai_builder to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart teai_builder to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
