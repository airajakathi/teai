...
from teai_builder.agent.tools.context import current_request_session_key
from teai_builder.agent.tools.exec_session import (
...
from teai_builder.security.workspace_access import current_scope_allows_loopback, current_tool_workspace
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to teai_builder internal state files (#2989).
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments = []
        if self.path_prepend:
            env["TEAI_BUILDER_PATH_PREPEND"] = self.path_prepend
            segments.append("$TEAI_BUILDER_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["TEAI_BUILDER_PATH_APPEND"] = self.path_append
            segments.append("$TEAI_BUILDER_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from teai_builder.security.network import contains_internal_url
