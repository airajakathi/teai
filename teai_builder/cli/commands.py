...
# Remove default handler and re-add with unified teai_builder format
logger.remove()
...
from teai_builder import __logo__, __version__  # noqa: E402
from teai_builder.agent.loop import AgentLoop  # noqa: E402
from teai_builder.cli.stream import StreamRenderer, ThinkingSpinner  # noqa: E402
from teai_builder.config.paths import get_workspace_path, is_default_workspace  # noqa: E402
from teai_builder.config.schema import Config  # noqa: E402
from teai_builder.utils.evaluator import evaluate_response  # noqa: E402
from teai_builder.utils.helpers import sync_workspace_templates  # noqa: E402
from teai_builder.utils.restart import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} TeAi Builder - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from teai_builder.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} TeAi Builder[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} TeAi Builder[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} TeAi Builder v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """TeAi Builder - autonomous AI software company."""
    pass
...
def onboard(
    ...
    """Initialize TeAi Builder configuration and workspace."""
    from teai_builder.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from teai_builder.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'teai_builder onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'teai_builder agent -m "Hello!"'
    gateway_cmd = "teai_builder gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} TeAi Builder is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from teai_builder.channels.registry import discover_all
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from teai_builder.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from teai_builder.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show teai_builder runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'teai_builder-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from teai_builder.api.server import create_app
    ...
    try:
        agent_loop = AgentLoop.from_config(
            ...
            audio_generation_provider_configs=audio_gen_provider_configs(runtime_config),
        )
...
def gateway(
    ...
    """Start the teai_builder gateway."""
    if verbose:
    ...
    if host is None:
        host = "0.0.0.0"
    _run_gateway(cfg, port=port, host=host, health_server_enabled=False)
...
def _run_gateway(
    ...
    *,
    ...
    from teai_builder.bus.queue import MessageBus
    from teai_builder.bus.runtime_events import RuntimeEventBus
    from teai_builder.channels.manager import ChannelManager
    from teai_builder.cron.bound_runner import run_bound_cron_job
    from teai_builder.cron.service import CronJobSkippedError, CronService
    from teai_builder.cron.session_turns import is_bound_cron_job
    from teai_builder.cron.types import CronJob
    from teai_builder.providers.audio_generation import audio_gen_provider_configs
    ...
    # Always force websocket to be enabled for the gateway
    ...
    async def _health_server(host: str, health_port: int):
        ...
        async def handle(reader, writer):
            ...
            # Forward to WebSocket channel's HTTP router
            ...
            if ws_channel:
                ...
                # Mock connection for the router
                ...
                mock_conn = MockConnection(writer)
                response = await ws_channel._dispatch_http(mock_conn, request)
                if response:
                    # If it's a Response object from websockets
                    if hasattr(response, "status_code"):
                        res_text = (
                            f"HTTP/1.0 {response.status_code} {response.reason_phrase}\r\n"
                        )
                        for k, v in response.headers.items():
                            res_text += f"{k}: {v}\r\n"
                        res_text += f"Content-Length: {len(response.body)}\r\n"
                        res_text += "\r\n"
                        writer.write(res_text.encode() + response.body)
                    elif response is not True: # Already handled by respond()
    ...
    async def run():
        ...
        try:
            ...
            if health_server_enabled:
                tasks.append(_health_server(host, port))
            if open_browser_url:
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show teai_builder runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from teai_builder.channels.registry import discover_all
    from teai_builder.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from teai_builder.channels.registry import discover_all, discover_channel_names
...
@app.command()
def status():
    """Show teai_builder status."""
    from teai_builder.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} TeAi Builder Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from teai_builder.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from teai_builder.providers.github_copilot_provider import login_github_copilot
