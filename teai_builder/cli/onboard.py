"""Interactive onboarding questionnaire for teai_builder."""
...
from teai_builder.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    from teai_builder import __logo__, __version__
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]teai_builder[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from teai_builder.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from teai_builder.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"teai_builder.channels.{name}")
