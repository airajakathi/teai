...
from teai_builder.audio.transcription import resolve_transcription_config
from teai_builder.audio.transcription_registry import (
    resolve_transcription_provider,
    transcription_provider_names,
)
from teai_builder.config.loader import get_config_path, load_config, save_config
from teai_builder.config.schema import ModelPresetConfig, ProviderConfig
from teai_builder.providers.image_generation import (
...
def _oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from teai_builder.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def login_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from teai_builder.providers.github_copilot_provider import (
