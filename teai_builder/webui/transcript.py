...
def replay_transcript_to_ui_messages(
    ...
    *,
    ...
    Mirrors the core fold in ``useTeaiBuilderStream.ts`` (delta, reasoning,
...
def collect_canvas_items(
    lines: list[dict[str, Any]],
    *,
    augment_content: Callable[[str, str], str | None] | None = None,
) -> list[dict[str, Any]]:
    """Extract canvas items pushed via ``canvas()`` so reload restores the panel.

    Transcript records persist the raw ``agent_ui`` blob (with the original file
    path). ``augment_content`` re-signs media/HTML paths into browser-loadable
    URLs at replay time, since signed URLs are process-local. Screenshot
    requests are skipped (they are transient capture prompts, not artifacts).
    """
    items: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    for rec in lines:
        ui = rec.get("agent_ui")
        if not isinstance(ui, dict) or ui.get("kind") != "canvas":
            continue
        data = ui.get("data")
        if not isinstance(data, dict):
            continue
        ...
        if not isinstance(ctype, str) or not isinstance(content, str) or not content:
            continue
        if ctype == "screenshot":
            continue
        out_content = content
        if augment_content is not None:
            signed = augment_content(ctype, content)
            if signed:
                out_content = signed
        key = (ctype, out_content)
        if key in seen:
            continue
        seen.add(key)
        item: dict[str, Any] = {"type": ctype, "content": out_content}
...
def build_webui_thread_response(
    ...
    *,
    ...
    augment_assistant_media: Callable[[list[str]], list[dict[str, Any]]] | None = None,
    augment_assistant_text: Callable[[str], str] | None = None,
    augment_canvas_content: Callable[[str, str], str | None] | None = None,
    ...
    canvas_items = collect_canvas_items(lines, augment_content=augment_canvas_content)
    if canvas_items:
        payload["canvas_items"] = canvas_items
    if page is not None:
