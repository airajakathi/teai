...
from teai_builder.apps.protocol import app_manifest, compact_dict
from teai_builder.config.paths import get_runtime_subdir
from teai_builder.security.workspace_policy import is_path_within
...
TEAI_BUILDER_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/teai_builder-extension/main/registry.json"
TEAI_BUILDER_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/teai_builder-extension/main"
_CATALOG_SOURCES = (
    ...
    ("extensions", TEAI_BUILDER_EXTENSION_REGISTRY_URL, TEAI_BUILDER_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "teai_builder-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "teai_builder-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["teai_builder_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from teai_builder."
        return f"""---
...
Use this skill when the user asks teai_builder to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_teai_builder_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- teai_builder-cli-app-note -->"
        if marker in content:
            return content
        name = str(app.get("name") or "unknown")
        note = f"""{marker}
## TeaiBuilder execution
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_teai_builder_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy != "bundled":
            ...
            if still_managed or (not managed_entry_path and still_available):
                ...
                message = (
                    ...
                    "so teai_builder kept it installed."
                )
                return self.payload() | {
                    "last_action": {
                        "ok": False,
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from teai_builder. {entry_point} "
                "is still available because it is managed outside teai_builder."
            )
