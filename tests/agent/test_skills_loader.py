"""Tests for teai_builder.agent.skills.SkillsLoader."""
...
def _write_skill(
    ...
    *,
    ...
    """Create ``base / name / SKILL.md`` with optional teai_builder metadata JSON."""
    skill_dir = base / name
    skill_dir.mkdir(parents=True)
    lines = ["---"]
    if metadata_json is not None:
        payload = json.dumps({"teai_builder": metadata_json}, separators=(",", ":"))
...
def test_list_skills_filter_unavailable_excludes_unmet_bin_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_bin",
        metadata_json={"requires": {"bins": ["teai_builder_test_fake_binary"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    def fake_which(cmd: str) -> str | None:
        if cmd == "teai_builder_test_fake_binary":
            return None
        return "/usr/bin/true"

    monkeypatch.setattr("teai_builder.agent.skills.shutil.which", fake_which)
...
def test_list_skills_filter_unavailable_includes_when_bin_requirement_met(
    ...
    def fake_which(cmd: str) -> str | None:
        if cmd == "teai_builder_test_fake_binary":
            return "/fake/teai_builder_test_fake_binary"
        return None
...
def test_list_skills_filter_unavailable_false_keeps_unmet_requirements(
    ...
    monkeypatch.setattr("teai_builder.agent.skills.shutil.which", lambda _cmd: None)
...
def test_list_skills_filter_unavailable_excludes_unmet_env_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_env",
        metadata_json={"requires": {"env": ["TEAI_BUILDER_SKILLS_TEST_ENV_VAR"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    monkeypatch.delenv("TEAI_BUILDER_SKILLS_TEST_ENV_VAR", raising=False)
...
def test_list_skills_openclaw_metadata_parsed_for_requirements(
    ...
    oc_payload = json.dumps({"openclaw": {"requires": {"bins": ["teai_builder_oc_bin"]}}}, separators=(",", ":"))
    ...
    monkeypatch.setattr(
        "teai_builder.agent.skills.shutil.which",
        lambda cmd: "/x" if cmd == "teai_builder_oc_bin" else None,
    )
    entries = loader.list_skills(filter_unavailable=True)
    assert entries == [
...
def test_get_skill_metadata_handles_yaml_types(tmp_path: Path) -> None:
    ...
    skill_dir.mkdir(parents=True)
    payload = json.dumps({"teai_builder": {"requires": {"bins": ["gh"]}, "always": True}}, separators=(",", ":"))
    skill_path = skill_dir / "SKILL.md"
    skill_path.write_text(
        "---\n"
        "name: typed\n"
