...
async def test_runner_estimates_usage_when_provider_omits_usage(monkeypatch):
    ...
    monkeypatch.setattr("teai_builder.agent.runner.estimate_message_tokens", lambda message: 7)
...
async def test_runner_calls_run_level_hooks_on_success():
    from teai_builder.agent.hook import AgentHook, AgentRunHookContext
    from teai_builder.agent.runner import AgentRunner, AgentRunSpec
