...
from teai_builder.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from teai_builder.config.loader import load_config, save_config
from teai_builder.config.schema import Config, ModelPresetConfig
from teai_builder.session import webui_turns as wth
from teai_builder.session.manager import SessionManager
from teai_builder.webui.gateway_services import GatewayServices, build_gateway_services
...
from teai_builder.webui.settings_api import settings_payload, update_provider_settings
from teai_builder.webui.transcript import append_transcript_object, read_transcript_lines
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "teai_builder.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-TeaiBuilder-Auth", "my-secret")])
...
async def test_webui_message_envelope_marks_inbound_metadata(bus: MagicMock) -> None:
    from teai_builder.webui.transcript import read_transcript_lines
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from teai_builder.webui.transcript import build_webui_thread_response, read_transcript_lines
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("teai_builder.webui.transcript.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
async def test_send_stages_external_media_as_signed_url(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("teai_builder.channels.websocket.get_media_dir", fake_media_dir)
    monkeypatch.setattr("teai_builder.webui.media_gateway.get_media_dir", fake_media_dir)
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"]}, bus, gateway=_basic_handler(bus))
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
...
def test_registry_discovers_websocket_channel() -> None:
    from teai_builder.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    monkeypatch.setattr(
        "teai_builder.webui.settings_api._oauth_provider_status",
        lambda _spec: {
            "configured": False,
            "account": None,
            "expires_at": None,
    ...
    try:
        ...
        assert workspace_path.endswith("/.teai_builder/workspace")
...
async def test_bootstrap_exposes_native_surface(bus: MagicMock) -> None:
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-TeaiBuilder-Auth": "native-secret"},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("teai_builder.config.loader._current_config_path", config_path)
    monkeypatch.setenv("TEAI_BUILDER_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_sessions_list_includes_active_run_started_at(monkeypatch) -> None:
    ...
    from teai_builder.session import webui_turns as wth
    from teai_builder.webui import ws_http as ws_http_module
...
def test_handle_webui_thread_get_returns_json(tmp_path, monkeypatch) -> None:
    ...
    from teai_builder.webui.transcript import append_transcript_object
...
def test_handle_file_preview_returns_workspace_file(tmp_path) -> None:
    ...
    workspace = tmp_path / "workspace"
    source = workspace / "teai_builder" / "agent" / "hook.py"
    source.parent.mkdir(parents=True)
    ...
    enc = quote(key, safe="")
    path = quote("teai_builder/agent/hook.py:12", safe="")
    req = Request(
...
def test_file_preview_normalizes_windows_file_url() -> None:
    from teai_builder.webui.file_preview import _clean_preview_path
