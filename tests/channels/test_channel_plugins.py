...
from teai_builder.config.loader import save_config
from teai_builder.config.schema import ChannelsConfig, Config
from teai_builder.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from teai_builder.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from teai_builder.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from teai_builder.channels.registry import discover_plugins
...
def test_discover_enabled_imports_only_enabled_builtins():
    from teai_builder.channels.registry import discover_enabled
    ...
    with (
        patch("teai_builder.channels.registry.load_channel_class", side_effect=_load_channel),
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "teai_builder.channels.registry.discover_enabled",
...
async def test_base_channel_reads_current_transcription_config_each_call(
    ...
    from teai_builder.providers import transcription as transcription_mod
...
async def test_base_channel_respects_disabled_transcription_config(
    ...
    with patch("teai_builder.providers.transcription.GroqTranscriptionProvider") as provider:
...
def test_openai_transcription_provider_honors_api_base_argument():
    from teai_builder.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("teai_builder.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("teai_builder.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "teai_builder.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from teai_builder.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("teai_builder.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("teai_builder.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("teai_builder.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("teai_builder.channels.manager.consume_restart_notice_from_env", return_value=notice):
