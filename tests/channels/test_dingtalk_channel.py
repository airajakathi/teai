...
try:
    from teai_builder.channels import dingtalk
...
import teai_builder.channels.dingtalk as dingtalk_module
from teai_builder.bus.queue import MessageBus
from teai_builder.channels.dingtalk import DingTalkChannel, DingTalkConfig, TeaiBuilderDingTalkHandler
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "TeaiBuilder Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = TeaiBuilderDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/teai_builder_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/teai_builder_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "teai_builder.config.paths.get_media_dir",
