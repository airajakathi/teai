"""Tests for the TeaiBuilder programmatic facade."""
...
from teai_builder.teai_builder import TeaiBuilder, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        TeaiBuilder.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = TeaiBuilder.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("teai_builder.config.loader.load_config") as mock_load, \
         patch("teai_builder.providers.factory.make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        TeaiBuilder.from_config()
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from teai_builder.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = TeaiBuilder.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert teai_builder.TeaiBuilder is TeaiBuilder
    assert teai_builder.RunResult is RunResult
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from teai_builder.bus.events import OutboundMessage
    from teai_builder.providers.base import ToolCallRequest
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from teai_builder.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from teai_builder.providers.base import ToolCallRequest
