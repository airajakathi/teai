...
import teai_builder.providers.base as provider_base
from teai_builder.providers.openai_codex_provider import (
    OpenAICodexProvider,
...
def _mock_codex_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "teai_builder.providers.openai_codex_provider.get_codex_token",
...
def _capture_codex_warnings(monkeypatch: pytest.MonkeyPatch) -> _WarningCaptureLogger:
    ...
    monkeypatch.setattr("teai_builder.providers.openai_codex_provider.logger", capture)
    return capture
...
async def test_codex_request_non_200_populates_http_metadata(monkeypatch) -> None:
    ...
    monkeypatch.setattr("teai_builder.providers.openai_codex_provider.httpx.AsyncClient", fake_client)
...
async def test_codex_request_honors_stream_idle_timeout_env(monkeypatch) -> None:
    """TEAI_BUILDER_STREAM_IDLE_TIMEOUT_S overrides the default Codex stream timeout."""
    monkeypatch.setenv("TEAI_BUILDER_STREAM_IDLE_TIMEOUT_S", "5")
    original_client = httpx.AsyncClient
    seen: dict[str, int] = {}
...
async def test_codex_prompt_cache_key_uses_stable_conversation_prefix(monkeypatch) -> None:
    ...
    monkeypatch.setattr("teai_builder.providers.openai_codex_provider._request_codex", fake_request)

    provider = OpenAICodexProvider()
    await provider.chat(
        [
            {"role": "system", "content": "You are teai_builder."},
